...
import { getOnAirSlideMenuFavoriteList } from "../../apis/OnAir/OnAirSlideMenuFavoriteApis";

const OnAirSlideCardFavorite = () => {
  ...
  console.log("좋아요 리스트",OnAirSlideMenuValue);

  useEffect (()=>{
    const OnAirSlideData = async () => {
      try {
        const res = await getOnAirSlideMenuFavoriteList();
        if(res){
...
export default OnAirSlideCardFavorite
